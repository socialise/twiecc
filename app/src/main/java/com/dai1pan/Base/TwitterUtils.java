...
//        return twitter;
    ...
    public static void storeAccessToken(final Context context, final AccessToken accessToken, final int useNumber) {
        ...
        Runnable run = new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    editor.putInt(LAST_USE, useNumber);
                    editor.commit();

                    final Twitter twitter = TwitterUtils.getTwitterInstance(useNumber);
                    final User user;
                    a = twitter.getId();
                    user = twitter.showUser(a);

                    editor = preferences.edit();
                    //アカウント切換用のタイトル文字をプリファレンスへ保存
                    editor.putString(ACCOUNT_NAME + useNumber, user.getScreenName());
                    editor.commit();
